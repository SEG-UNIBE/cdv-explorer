...
```json
{
  ...
  "insights": {
    ...
    "changes_in_status": [/* ... */],
    "interrelations": {
      "preamble_extracted":   [{target, type}],
      "body_extracted_regex": [{target, count}],
      "body_extracted_llm":   [{run_id, model, timestamp, status,
                                dependencies: [{target, evidence, reason}]}]
    }
  }
}
```

All `target` values use source-qualified `source_slug:id` keys, so references across IP catalogs of the same ecosystem (e.g. a BIP depending on a SLIP) are expressible.
`body_extracted_llm` is an append-only list of timestamped extraction runs — consumers resolve it to the latest successful run. Prompt provenance is stored once per source/snapshot in the shared LLM run manifest.
...
[^2]: Bögli, R. and Kehrer, T. _CDV-Explorer: Navigating Improvement Proposal Spectra in Decentralized OSS Ecosystems._ In Companion Proc. Int'l Conf. on Software and Systems Reuse, Product Lines, and Configuration (VARIABILITY), Limassol, Cyprus, Sep. 2026. DOI: forthcoming. [[Preprint]](https://romanboegli.ch/assets/pdf/Boegli_2026_CDVExplorer_Tool.pdf)
