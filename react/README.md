...
- The lifecycle scripts also run `scripts/syncPublicData.js`, which copies only the
  Stage IV frontend payloads (the per-snapshot JSON files under `04_postprocess/`,
  loaded by `src/data.js`) from `../ip_data` into
...
- The dashboard fetches payloads in two phases: `network_data.json`,
  ...
  (proposal nodes feed every section). Dependency metrics and consistency,
  centrality, evolution, and conformity payloads are fetched only when their
  dashboard section scrolls into view (`src/dashboard/useSectionDataLoader.js`).
