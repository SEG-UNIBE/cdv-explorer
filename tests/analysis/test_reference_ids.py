...
class ReferenceIdTests(unittest.TestCase):
    ...
    def test_normalize_reference_id_decimal(self) -> None:
        self.assertEqual(
            normalize_reference_id(
                "0032", proposal_label="BIP", reference_pattern=r"\bBIP[-#\s]?(\d+)\b"
            ),
            "32",
        )

    def test_normalize_reference_id_decimal_rejects_negative_and_non_numeric(
        self,
    ) -> None:
        self.assertIsNone(
            normalize_reference_id(
                "-1", proposal_label="BIP", reference_pattern=r"\bBIP[-#\s]?(\d+)\b"
            )
        )
        self.assertIsNone(
            normalize_reference_id(
                "BIP 32", proposal_label="BIP", reference_pattern=r"\bBIP[-#\s]?(\d+)\b"
            )
        )
