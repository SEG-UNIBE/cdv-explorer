...
_BIP_CONFIG = {
    ...
    "preamble": {
        ...
        "expected_headlines": {"abstract": 2, "motivation": 2, "specification": 2, "copyright": 2},
...
class BipExtractionTests(unittest.TestCase):
    def test_extracts_mediawiki_preamble_and_normalizes_numeric_id(self):
        with TemporaryDirectory() as tmp:
            ...
            harvest_dir.mkdir()
            (harvest_dir / "bip-0002.mediawiki").write_text(_bip_content("0002"), encoding="utf-8")

            extract(_BIP_CONFIG, harvest_dir, output_dir)

            output = json.loads((output_dir / "bip-0002.json").read_text(encoding="utf-8"))
    ...
    def test_extracts_markdown_bip_files_and_skips_non_matching_markdown(self):
        with TemporaryDirectory() as tmp:
            ...
            (harvest_dir / "bip-0379.md").write_text(_bip_content("0379"), encoding="utf-8")

            extract(_BIP_CONFIG, harvest_dir, output_dir)

            output_files = sorted(path.name for path in output_dir.iterdir())
            output = json.loads((output_dir / "bip-0379.json").read_text(encoding="utf-8"))
    ...
    def test_prunes_stale_bip_json_after_extracting_current_files(self):
        with TemporaryDirectory() as tmp:
            ...
            harvest_dir.mkdir()
            output_dir.mkdir()
            (harvest_dir / "bip-0002.mediawiki").write_text(_bip_content("2"), encoding="utf-8")
